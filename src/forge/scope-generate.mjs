...
// Scope-Forge generator: propose a per-file behavioural check for the ONE changed file `rel` of a
// gamed->honest recovery. Unlike the single-file generateCandidates (which reads two whole files), this
...
export function buildScopeGeneratorPrompt({ goal, rel, goodContent, badContent, critique, scorerCatalog }) {
  ...
  return [
    ...
    'the gate but was GAMED. Exactly ONE file changed between the honest and gamed versions. Propose checks on',
    `that file that PASS the HONEST version and FAIL the GAMED one. Do NOT include the file path in your args —`,
    'it is added automatically; pass only the behavioural arguments.',
    '',
    `Goal: ${goal}`,
    `Changed file: ${rel}`,
    '',
...
export async function scopeGenerateCandidates({ goal, goodArtifact, badArtifact, rel, critique = '', scorerCatalog, allowlist, propose, maxCandidates = 5, model }) {
  const goodContent = safeRead(join(goodArtifact, rel))
  const badContent = safeRead(join(badArtifact, rel))
  const prompt = buildScopeGeneratorPrompt({ goal, rel, goodContent, badContent, critique, scorerCatalog })
