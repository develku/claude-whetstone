# Whetstone Dogfood — External Target

> This is whetstone's own quality process — the project is improved by running its measured loop on an external target (an external open-source project).

A **meta loop** that runs whetstone on its own codebase and beyond (dogfooding): use whetstone on a *real external project*, capture
the friction during the upgrade, and feed it back to improve whetstone itself.

```
A) whetstone upgrades the external target   (real external target — not whetstone's own gate)
B) collect friction artifacts               (where whet helped / where it fought the maintainer)
...
the bootstrap was only proven on whetstone's *own* `$0` npm-test gate. The external target is the **first
real external run on its own codebase (dogfooding)** — the missing evidence class. Confirmed whetstone changes graduate into
...
|---|------|--------|------|-------|---------|------------|
| 1 | 2026-06-30 | external target recall: group index by type | `uv run pytest` (RED authored) | sonnet | **DONE @ pass 1, verified** | 122,067 tok / $0.29 |
| 2 | 2026-06-30 | external target autocapture.scan (marker extraction + injection gate) | `pytest tests/test_autocapture.py` (RED + injection suite) | sonnet | **DONE @ pass 1, verified; PR-review caught 1 untested-path bug** | 304,948 tok / $0.43 |

---

## Run 1 — recall index grouped by type

**Target:** the external target @ branch `feat/recall-grouped-index`
...
- **F1 — gate authoring is 100% manual and demands adversarial thinking.** whetstone's whole
  value is bounded by gate quality, yet it gives the maintainer *no help* authoring or
...
- **F5 — LOW — driver mode does no built-in adversarial gaming-check or diff surfacing.** The
  maintainer must independently re-run the suite and read the diff to trust "done"; the driver prints
...
- **F4 → FIXED + graduated (`findings-register.md` DF-01).** Made the bundled `test-pass-rate.mjs`
  ...
  **End-to-end proof:** the *bundled* (not prototype) scorer on the real external target's suite →
...
## Run 2 — autocapture.scan (marker-driven Stop hook for the external target)

**Target:** the external target @ branch `feat/auto-capture-hook` (an external PR)
...
+ path-traversal rejection. Process-critical (auto-write to canon) → **a cross-model design review** preceded.
...
- **F6 — MEDIUM — a gate that mocks the boundary doesn't test the boundary wiring.** The unit gate
  ...
  path uncovered**, and whet guessed a non-existent canon API for it. The gate was 10/10
...
- **F7 — observation — for fuzzy/process-critical tasks, design dominates and whet is the cheap part.**
  Run 1 (recall) was ~all-whet. Run 2 needed brainstorm → cross-model design review → spec → plan → a human-authored
...
|----|-------------|-----------|
| **F4** | **FIXED** (DF-01, an external PR) | bundled scorer made pytest-portable; re-validated in run 2. |
...
| **F5** | **DEFERRED — low** | driver mode prints only score+trajectory (no built-in adversarial gaming-check/diff surfacing). The maintainer verifies manually; acceptable by design. Revisit if first-time-user trust friction recurs. |

**State: stable.** All run-1/run-2 work committed + pushed; 3 PRs open (two on the external target, one on whetstone);
every finding dispositioned. The only remaining actions are maintainer-owned: merge the PRs, and
reinstall the external target's global bin post-merge so the Stop hook can reach `capture-scan`.
