...
4. **Coverage ≥ baseline** (soft ratchet — a MANUAL comparison; no automated gate reads it).
   Run `npm run coverage`; the `all files` **line / function** aggregates (stable, exact) must not
   drop below the register's recorded baseline. **Branch is a reproducible-MINIMUM floor, not an exact
   value:** it jitters ~±0.09 run-to-run (spawn-race branch flips on already-covered lines in
   `converge.mjs`, `act-claude.mjs`, and the invariant `loop.mjs`), so a single run landing inside the
   recorded jitter band is NOT a regression — only a drop below the floor is. Re-run before treating a
   sub-floor branch number as a real drop. (Cycle-5 QL-01.)
...
| 5 | 2026-07-02 | all four (re-probe AUD-04 + AUD-01) | 0 (of 2 candidates) | 1 (QL-01 doc-calibration) | 96.42 / 84.03 / 94.22 | Adversarial re-probe of the two freshest security fixes + 4-axis discovery (6-agent Workflow: 4 parallel axis finders + refute-by-default verifiers, **no-web**, empirical). **All axes NEGATIVE — boundaries sound**, independently reproduced by a main-loop probe harness that ran hostile artifacts through the real `runIsolated`. AUD-04 egress: 12 egress/nonce hypotheses refuted (socket builtins + `data:`-nest + network-import URL + WASI + nonce-recovery + `process.report` all closed). AUD-01 decompose: `SHELL_SCORERS` is EXACTLY the shell-reaching set, `shq` robust across the two-hop path, containment segment-correct. Both surfaced candidates REFUTED by verify — COV-JITTER framing wrong (the ratchet is soft/manual; no gate trips) and SIMP-1 tolerated-dup. **Zero code changes** (strong convergence continues). Sole action: QL-01 branch-floor recalibration 84.12 → 84.03 (reproducible minimum; jitter from `converge.mjs`/`act-claude.mjs` + invariant `loop.mjs` documented), operator-approved. Docs-only; 8 invariant files byte-untouched; gate green 1067/1067. |
| 6 | 2026-07-12 | all four (post-v1.13.0 surface) | 5 (of 9 candidates) | 5 | 96.76 / 84.03 (obs 85.10) / 94.57 | First pass over the ~1300 lines added since cycle 5 (v1.9→v1.13). 22-agent Workflow (4 parallel no-web empirical axis finders + 2-lens refute-by-default verify) + Fable firsthand diff read. Plan by Fable, fixes run by Opus executor agents (TDD, verified firsthand, one commit each). **The brand-new `blast-radius` module carried the only HIGH** (C6-01: the capped-window slide → the containment control `rmSync`-destroys pre-existing untracked files it never snapshotted, mislabelled `reverted`) + a MEDIUM crash (C6-02: uncaught ENOENT out of the `finally` on a nested restore). Plus C6-03 (`--gate-self-probe` missing the `--observe` skip its sibling `--gate-audit` has → paid store-pollution), C6-04 (judge-spend twin-persist drift — scope/converge silently under-charged the budget; root-caused via a shared `passSpend` helper), C6-05 (doc-coverage tilde-fence anti-gaming bypass). 4 rejected by adversarial verify (false symlink-security escalation, harmless isAuthFailure false-positive, 2 tolerated-dup framings). The invariant `loop.mjs` (AUD-05) confirmed a clean deliberate hash bump. Gate green 1248/1248 throughout; 8 invariant files byte-untouched; both doc gates 100. **Lesson: the newest surface, not the hardened core, is where the next cycle's HIGH lives.** |
