...
export const TAXONOMY = Object.freeze([
  ...
  },
  {
    ...
    proof: null,
    status: 'RED',
    notes: 'UNGUARDED GAP. gateVerdict (src/gate.mjs:28) reads only scores.at(-1) and stops at the first latest>=target; no repeated-run, variance, or lower-confidence-bound check. Demonstrated by the gap-demo in test/taxonomy.test.mjs (history [40,40,100] -> done). The fix is the Confidence Gate (risk-based done): repeat/quorum the scorer and gate on a lower-confidence bound. Closing this must flip the gap-demo and update this entry to GREEN with its proof.',
  },
  {
    n: 6,
