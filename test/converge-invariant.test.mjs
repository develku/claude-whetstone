...
const INVARIANT = {
  ...
  // loop.mjs hash DELIBERATELY updated 2026-07-03 (v1.9.0, AUD-05): after a keep-best restore the loop now
  // re-points last_critique at best_critique + stamps restored_at_pass + save()s, so the next edit is steered
  // by the live (restored) artifact, not the reverted dead version. Stamp is BEFORE verifyDone so a later
  // stability/confirm veto legitimately overwrites. DCA 20260703T203705 (REFINED-AND-PROCEED, Option A);
  // all pre-existing loop/restore tests pass unchanged. (Prior deliberate bump: 2026-07-02 v1.6.0 escalation ladder.)
  'src/loop.mjs': '17eb8fff1e872a3d8b11314fdfc4064434a308b3db3e7f1dff48d286c622ebc6',
