...
//   WHET_FAKE_MODE   success (default) | max_turns | fatal | auth | hang
...
if (mode === 'hang') {
  ...
  const isFatal = mode === 'fatal' || mode === 'auth'
  const result = {
    type: 'result',
    subtype: mode === 'max_turns' ? 'error_max_turns' : isFatal ? 'error_during_execution' : 'success',
    is_error: isFatal,
    total_cost_usd: cost,
    ...
    ...(mode === 'auth' ? { api_error_status: 401, result: 'Unauthorized' } : {}),
  }
  process.stdout.write(JSON.stringify([{ type: 'system', subtype: 'init' }, result]))
  process.exit(mode === 'success' ? 0 : 1) // max_turns + fatal + auth all exit non-zero, like the real CLI
}
